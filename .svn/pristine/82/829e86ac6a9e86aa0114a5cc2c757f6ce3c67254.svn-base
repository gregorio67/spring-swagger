...
import com.lgcns.api.helper.RestHelper;
...
//	@ApiResponses(value = {@ApiResponse(code = 200, message = "OK"),@ApiResponse(code=404, message = "Not found user")})
	@RequestMapping(value="/v1/user/{userId}", method= {RequestMethod.POST}, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<BaseMap> userInfo(@PathVariable("userId") String userId) throws Exception {
        ...
        BaseMap result = RestHelper.exchange(url, params, BaseMap.class);
        if (result != null) {
